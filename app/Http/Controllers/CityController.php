...
class CityController extends Controller
{
    private function generate_citites()
    {
        ...
        $values = array_map(function($v) { return $v[0]; }, array_filter($adt->used_values, function($v) { return count($v) > 0; }) );
    ...
    public function map(Request $request) {
        ...
        foreach( $all_dets['aDetails'] as $adt ) {
            ...
            if( Auth::user()->can('viewNetworkDetails', $adt->identity) )
                $cities_array[$cities_dict[$adt->value[0]]]['alumni'][] = $adt->identity;
            else
                $cities_array[$cities_dict[$adt->value[0]]]['alumni'][] = ['id' => -1];
        }

        $cities_array = array_filter($cities_array, function($v) { return count($v['alumni']) > 0; });
