...
export default function AddBlock({ alwaysVisible = false, addBlock }) {
    ...
    return <div ref={addRef} className="relative">
        ...
        <div className={"absolute right-0 left-auto border border-black rounded-xl my-2 " + (menuOpen ? "visible" : "invisible")}>
            <div className="flex flex-col z-10 flex-wrap max-h-[60vh] basis-0 w-max">
                {Object.entries(RegisteredTools).map(([k, v]) =>
                    <div className="flex flex-row items-center gap-2 drawer-item-noborder rounded-xl" key={k} onClick={() => onClick(k, v)}>
